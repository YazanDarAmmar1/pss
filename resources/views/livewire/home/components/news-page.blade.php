    ...
    <div class="container">
        ...
        @if($news)
            ...
            <div class="box box1 pt-30">
                <a href="{{route('news.details', $firstNews->id)}}" class="item item-1 pb-40 d-flex flex-column-p gap-50 gap-40-p body-color">
                    <figure class="fig d-flex shrink-0 w-50 w-100-p">
                        <img src="{{ asset('storage/' . $firstNews->image_path) }}" class="object-fit radius-24" width="100%" height="297" />
                    </figure>
            ...
            <div class="box do-only box2 grid-2 grid-1-p gap-50 pt-40">
                @foreach($otherNews as $newsItem)
                    <a href="{{route('news.details', $newsItem->id)}}" class="item item-2 d-flex gap-50 body-color flex-column-p">
                        <figure class="fig d-flex shrink-0 w-50 w-100-p">
                            <img src="{{ asset('storage/' . $newsItem->image_path) }}" class="object-fit radius-24" width="234" height="234" />
                        </figure>
